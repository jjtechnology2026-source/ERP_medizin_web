...
export default function OrderFilters({ filters, onFiltersChange, onReset }: FiltersProps) {
  ...
  return (
    <div className="flex flex-wrap gap-4 items-center">
      ...
      <select
        className={filterClass}
        value={filters.type_sale}
        onChange={(e) => onFiltersChange({ ...filters, type_sale: e.target.value })}
      >
      ...
      <button
        onClick={onReset}
        className="flex items-center gap-2 text-red-500 text-sm font-semibold hover:bg-red-50 px-3 py-2 rounded-lg transition-colors"
      >
        <HiOutlineRefresh className="rotate-180" /> Restablecer filtro
      </button>
    </div>
  );
}