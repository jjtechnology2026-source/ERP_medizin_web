...
const loadFromAuthCatalog = () => {
  const catalog = useAuthStore.getState().medicinesCatalog;
  if (catalog?.length) {
    useProductsStore.getState().fetchInventory(true);
  }
};

if (typeof window !== "undefined") {
  useAuthStore.subscribe((state) => {
    if (state.medicinesCatalog?.length && !useProductsStore.getState().isInitialLoad) {
      useProductsStore.getState().fetchInventory(true);
    }
  });
}
...
export const useProductsStore = create<ProductsStore>()((set, get) => ({
  ...
  fetchInventory: async (force?: boolean) => {
    ...
    try {
      ...
      (localCatalog.length ? localCatalog : []).forEach((item: any) => {
      ...
      // API is optional — like Dart, inventory comes from login catalog + MQTT sync
    }
  ...
  saveMedicine: async (medicine) => {
    ...
    try {
      ...
      // API save is best-effort. Dart does MQTT + local state only.
    }
    if (editMode) {
      set({
  ...
  getFilteredInventory: () => {
    ...
    // Show all stock products so that out-of-stock items can be managed/replenished
