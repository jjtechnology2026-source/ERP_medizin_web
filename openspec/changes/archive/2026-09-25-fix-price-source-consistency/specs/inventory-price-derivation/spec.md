# Delta for Inventory Price Derivation
...
## Verification Inputs (Not Requirements)

These MUST be resolved against real data during apply; no value is asserted here:
- The actual `pharmacy_inventory`/`products` rows for `BOTELLON DE AGUA RECARGA` (price, base_price, profit_percentage, lote, vat) — required to confirm provenance.
- The meaning of `11` and `7Frasco` in the panel screenshot — required to distinguish a single-row product from a lot rows straddling a listing page.

## Open Decision For Design

- Whether the derivation input set includes `discount` in the persisted `price` (today `price` excludes discount while the panel preview applies it at `StockFeaturesForm.tsx` discounted price). The invariant as written covers `base_price`, `profit_percentage`, `vat` only; DESIGN MUST confirm or extend it.
